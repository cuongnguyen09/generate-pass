...
function TodoForm({ addTodo, todos }) {
  ...
  const [priority, setPriority] = useState("LEL");
  const [country, setCountry] = useState("VN");
  const [machine, setMachine] = useState("L");
  const [operation, setOperation] = useState("W");
  ...
  const onFormSubmit = (e) => {
    ...
    if (title !== "") {
      const newTodo = {
        id: uuidv4(),
        bios: md5(title).substring(0, 14),
        hostname: `${priority}-${country}-${machine}-${operation}-${Math.floor(1000 + Math.random() * 9000)}`
      }
